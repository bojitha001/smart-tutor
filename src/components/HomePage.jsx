...
const faqData = [
  ...
  },
  {
    ...
    answer: "We offer tutoring for a wide range of subjects including Mathematics, Science, Languages, Humanities, and more.",
  },
  {
    id: 5,
    question:"How do I schedule a session?",
    answer: "You can schedule a session by selecting a tutor and choosing a convenient time slot from their availability calendar." ,
  },
  {
    id: 6,
    question: "Do you offer group tutoring?",
    answer: "Yes, we provide group tutoring sessions at discounted rates. You can also join existing Kuppi Groups or create your own.",
  },
];
    ...
    <>
      ...
      <div className={`${styles["container-1"]}`}>
        <div className={`${styles["container-2"]}`}>
          ...
          <img className={`${styles["Main-Img"]}`} src={mainImage} alt="Students learning" />
        </div>
      </div>
      ...
      <div className={`${styles["categories-container"]}`}>
        ...
        <button className={`${styles["see-more-button"]}`} aria-label="See more categories">
      ...
      <div className={`${styles["testimonials-container"]}`}>
        ...
        <div className={`${styles["testimonials-slider"]}`}>
          <button className={`${styles["nav-button"]} ${styles["prev"]}`} onClick={goToPrevious} aria-label="Previous testimonial">
            ←
          </button>
          ...
          <div className={`${styles["testimonials-wrapper"]}`}>
            {windowWidth <= 768 ? (
              ...
              <div key={testimonials[currentIndex].id} className={`${styles["testimonial-card"]} ${styles["active"]}`}>
                <div className={`${styles["avatar"]}`}>
                  <img src={testimonials[currentIndex].image} alt={testimonials[currentIndex].name} />
                </div>
                <h2>{testimonials[currentIndex].name}</h2>
                <p className={`${styles["role"]}`}>{testimonials[currentIndex].role}</p>
                <p className={`${styles["testimonial-text"]}`}>{testimonials[currentIndex].text}</p>
              </div>
            ) : (
              ...
              [...Array(3)].map((_, i) => {
                ...
                return (
                  <div key={testimonials[index].id} className={`${styles["testimonial-card"]} ${styles[position]}`}>
                    <div className={`${styles["avatar"]}`}>
          ...
          <button className={`${styles["nav-button"]} ${styles["next"]}`} onClick={goToNext} aria-label="Next testimonial">
            →
          </button>
        </div>
      </div>
