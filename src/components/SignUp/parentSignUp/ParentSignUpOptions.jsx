...
export const ParentSignUpOptions = () => {
    ...
    const saveUserToFirestore = async (user) => {
        if(!userData) return;
        const parentRef = doc(db, "ParentDetails", user.uid); // Create a reference to the tutor collection
        const userRef = doc(db, "UserDetails", user.uid); // Create a reference to the tutor collection
        const parentExist = await getDoc(parentRef);
        if(parentExist.exists()){//Checks whether an account with the respective email already exists
        ...
        await setDoc(parentRef, userDataToSave);
