...
import styles from "../../../.ExternalCss/StudentSignUpOptions.module.css";
import smartTutorImage from "../../../assets/images/smartTutor.svg";
import signUpStudentImg from "../../../assets/images/SignupSt.png";
...
export const StudentSignUpOptions = () => {
  ...
  const saveUserToFirestore = async (user) => {
    if (!userData) return;
    const studentRef = doc(db, "StudentDetails", user.uid); // Create a reference to the student collection
      ...
      <div className={`row g-3 m-2 p-3 ${styles["grid"]}`}>
        ...
        >
          <div>
            ...
            <h3 className={`${styles["sub-titles-signUp"]}`}> Your Skills With Us!</h3>
          </div>
          <br />
          ...
          <img className={`${styles["signUp-Img"]}`} src={signUpStudentImg} alt="" />
        </div>
